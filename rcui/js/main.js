window.onload = () => {
    ...
    document.querySelectorAll("svg a").forEach((node) => {
        console.log(node.href.baseVal);
        node.addEventListener("click", () => {
            console.log("Pressed " + node.href.baseVal);
            let code = node.href.baseVal.substr(1);
            if (node.classList.contains("work")) {
                return; // Already in progress
            }
            node.classList.add("work");
            fetch("http://10.20.30.35/irSend?NEC=" + code, {
                    cache: 'no-cache',
                    mode: 'no-cors'
                })
                .then((response) => {
